  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const leaderboardResponse = await fetch('http://192.168.0.6:8000/game-scores/leaderboard');
        const leaderboardResult = await leaderboardResponse.json();
        setLeaderboardData(leaderboardResult.data);

        // Fetch user's total
        if (parsedUserData.id) {
          const userTotalResponse = await fetch(`http://192.168.0.6:8000/game-scores/user/${parsedUserData.id}/total`);
